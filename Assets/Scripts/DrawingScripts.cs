...
public class DrawingScripts : MonoBehaviour
{
    ...
    public Slider slidLine;
    public Slider slidCtrast;
    ...
    public TextMeshProUGUI cdown;
    public GameObject pnSnap;
    public GameObject pnTool;
    public AudioSource auSrc;
    public Image imProg;    
    public RawImage aUsDraw;
    ...
    private void Awake()
    {
        ...
        int delayTime = 200;
        var onSliderLineValueStream = slidLine.onValueChanged.AsObservable();
        onSliderLineValueStream.Sample(TimeSpan.FromMilliseconds(delayTime)).Subscribe((float f) => { OnLineSliderValueChange(slidLine); });
        var onSliderContrastValueStream = slidCtrast.onValueChanged.AsObservable();
        onSliderContrastValueStream.Sample(TimeSpan.FromMilliseconds(delayTime)).Subscribe((float f) =>
        {
            OnContrastSliderValueChange(slidCtrast);
        });
    ...
    IEnumerator loadCameraAndModel()
    {
        ...
        else if (drawMode == DRAWMODE.DRAW_SPECIAL)
        {           
            if(spineMode == SPINE.SATAN)
            {
                TextAsset asset = Resources.Load("satan") as TextAsset;
        ...
        else if (drawMode == DRAWMODE.DRAW_IMAGE)
        {
            texModelW = texModel.width;
            texModelH = texModel.height;
        }
        ...
        athreshold.setParameter(slidLine.value);
    ...
    {
        while (eSys && eSys.GetComponent<TouchScriptInputModule>() == null)
        {
            yield return null;
        }
        eSys.GetComponent<TouchScriptInputModule>().enabled = false;
    }
    ...
    IEnumerator Worker()
    {
        while (true)
        {
            ...
            if (wcHelper.IsPlaying() && wcHelper.DidUpdateThisFrame())
            {
                ...
                Utilities.Log("Display Mat have width = {0}, height = {1}", displayMat.width(), displayMat.height());
                if (isRecording)
                {
                    numberFrame++;
    ...
    {
        if(!focus && Everyplay.IsRecording())
        {
    ...
    IEnumerator CountDown()
    {
        ...
        wcHelper.Play();
        pnSnap.SetActive(true);
        pnTool.SetActive(false);
    ...
    void isolateBoundary()
    {
        ...
        //Animation
        aUsDraw.gameObject.transform.position = goModel.transform.position;
        aUsDraw.rectTransform.sizeDelta = rimgmodel.rectTransform.rect.size;
        aUsDraw.rectTransform.localScale = rimgmodel.rectTransform.localScale;
        ...
        aUsDraw.texture = areaUDrawTex;
        aUsDraw.gameObject.SetActive(true);
        //---------------------------------
        ...
        var widthUserDraw = aUsDraw.rectTransform.sizeDelta.x;
        ...
        seq.append(() =>
        {
            ...
            LeanTween.move(aUsDraw.gameObject, newPos, 2f);            
            LeanTween.scale(aUsDraw.gameObject, new Vector3(scal_, scal_, scal_), 2f);
        });
        seq.append(2);
        seq.append(() =>
        {            
        ...
        seq.append(() =>
        {
            ...
            spine.loop = true;
            aUsDraw.gameObject.SetActive(false);

            var anim = chrimasSong.clip.length - 5 + 5;
            GVs.timeAni = anim > GVs.timeAni ? GVs.timeAni : anim;
    ...
    IEnumerator SaveMasterPiece2()
    {
        ...
        imProg.GetComponent<RectTransform>().eulerAngles = Vector3.zero;
        LeanTween.rotateAround(imProg.gameObject, Vector3.forward, 360, 1)
            .setOnStart(() => { imProg.gameObject.SetActive(true); })
        ...
        Observable.WhenAll(cutvideo)
            ...
            {
                imProg.gameObject.SetActive(false);
                preTexCam = true;
